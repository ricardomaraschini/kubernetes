...
func (kl *Kubelet) SyncPod(ctx context.Context, updateType kubetypes.SyncPodType, pod, mirrorPod *v1.Pod, podStatus *kubecontainer.PodStatus) (isTerminal bool, err error) {
	...
	// XXX this has been added to allow the pod to start only when a given annotation is not present.
	if _, ok := pod.Annotations["kubernetes.io/reservation"]; ok {
		kl.recorder.Eventf(pod, v1.EventTypeNormal, events.StartDeferred, StartDeferredMsg)
		return false, fmt.Errorf("pod %q is considered a reservation", klog.KObj(pod))
	}
